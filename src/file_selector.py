...
class FileSelector:
    ...
    def select_files_interactive(self, custom_dir: str = None) -> List[Path]:
        """
        Interactive file selection interface with Excel file detection.
        
        Workflow:
        1. Determines search directory (custom or default input/)
        2. Scans for Excel files (.xlsx, .xls)
        3. Displays files with metadata (size, modification date)
        4. Prompts user for selection
        5. Returns selected file paths
        
        Args:
            custom_dir: Optional custom directory to search (overrides self.input_dir)
                       Example: "output" for content generation mode
        
        Returns:
            List[Path]: List of selected Excel file paths
                       Empty list if no files found or user cancels
        """
        # Determine search directory: custom parameter or default input directory
        # Example: custom_dir="output" searches in output/ folder
        search_dir = Path(custom_dir) if custom_dir else self.input_dir
        search_dir.mkdir(exist_ok=True)
        
        # Temporarily set input_dir for _get_excel_files method
        # This allows reusing _get_excel_files() which relies on self.input_dir
        original_dir = self.input_dir
        self.input_dir = search_dir
        
        # Get available Excel files from search directory
        # Returns list of dicts with file metadata (path, name, size, modified date)
        files = self._get_excel_files()
        
        # Restore original directory to maintain class state
        # Important: This happens before error check to ensure consistency
        ...
        if not files:
            ...
            print("="*60)
            print(f"\nPlease copy your Excel files to:")
