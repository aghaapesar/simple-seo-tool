...
class AIProcessor:
    ...
    def cluster_keywords(self, keywords: List[str]) -> List[Dict[str, Any]]:
        ...
        system_prompt = """شما یک متخصص استراتژی محتوای SEO برای زبان فارسی هستید. وظیفه شما گروه‌بندی کوئری‌های جستجو در کلاسترهای موضوعی است که برای تولید مقالات مجزا منطقی باشند. در تحلیل خود به ویژگی‌های خاص جستجوی فارسی، نگارش‌های مختلف، و intent کاربران ایرانی توجه کنید. خروجی را فقط به صورت JSON معتبر برگردانید."""
...
6. **نوع محتوا** - یکی از: راهنما/آموزش/مقایسه/لیست/تحلیل
7. **Search Intent** - یکی از: informational/commercial/transactional/navigational
...
- فقط کلیدواژه‌های لیست ورودی را استفاده کن"""
    ...
    def generate_content_improvements(
        ...
        system_prompt = """شما یک متخصص بهینه‌سازی محتوای SEO برای زبان فارسی هستید. وظیفه شما ارائه پیشنهادات مشخص و عملی برای بهبود عملکرد محتوا در نتایج جستجوی گوگل است. در تحلیل خود به ویژگی‌های خاص الگوریتم گوگل برای محتوای فارسی، رفتار کاربران ایرانی، و بهترین شیوه‌های SEO فارسی توجه کنید. خروجی را فقط به صورت JSON معتبر برگردانید."""
